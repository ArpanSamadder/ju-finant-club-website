...
async function getLegacyCards() {
  try {
    ...
    return [...cmsCards, ...fillers].slice(0, 6);
  } catch {
...
export default async function HomePage() {
  ...
  const renderLegacyCard = (card: LegacyCard, index: number, group: 'primary' | 'duplicate') => (
    <article key={`${group}-${card._id ?? card.title}-${index}`} aria-hidden={group === 'duplicate' ? 'true' : undefined} className="legacy-card group relative overflow-hidden rounded-[14px] border border-[#5F79FF]/90 bg-[#030817]/92 p-[1px] shadow-[0_18px_48px_rgba(0,0,0,.62)] transition duration-500 hover:-translate-y-1.5 hover:border-[#00D9FF] hover:shadow-[0_24px_60px_rgba(0,0,0,.68)]">
      ...
      <section id="legacy-foundation" className="relative min-h-[calc(100svh-72px)] overflow-hidden bg-[#020817] px-6 py-[7vw]">
        ...
        <div className="relative z-10 mx-auto max-w-[1680px]">
          ...
          <div className="relative mt-[4.2vw] px-20 max-2xl:px-16 max-xl:px-10 max-lg:px-0">
            <button aria-hidden="true" tabIndex={-1} className="absolute left-0 top-1/2 z-20 flex h-12 w-12 -translate-y-1/2 items-center justify-center rounded-full border border-[#115FEB]/80 bg-[#020817] text-3xl font-light leading-none text-white shadow-none transition hover:border-[#00D9FF] hover:text-[#00D9FF] max-xl:h-11 max-xl:w-11 max-xl:text-2xl max-lg:hidden">
              ‹
            </button>
            <button aria-hidden="true" tabIndex={-1} className="absolute right-0 top-1/2 z-20 flex h-12 w-12 -translate-y-1/2 items-center justify-center rounded-full border border-[#115FEB]/80 bg-[#020817] text-3xl font-light leading-none text-white shadow-none transition hover:border-[#00D9FF] hover:text-[#00D9FF] max-xl:h-11 max-xl:w-11 max-xl:text-2xl max-lg:hidden">
              ›
            </button>

            <div className="legacy-marquee-viewport">
              <div className="legacy-marquee-track">
                <div className="legacy-marquee-group">
                  {legacyCards.map((card, index) => renderLegacyCard(card, index, 'primary'))}
                </div>
                <div className="legacy-marquee-group" aria-hidden="true">
                  {legacyCards.map((card, index) => renderLegacyCard(card, index, 'duplicate'))}
                </div>
              </div>
            </div>
          </div>
